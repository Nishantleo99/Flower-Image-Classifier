...
def load_data(where="./flowers"):
    ...
    vloader = torch.utils.data.DataLoader(validation_datasets,batch_size=32,shuffle=True)
    testloader = torch.utils.data.DataLoader(test_datasets,batch_size=20,shuffle=True)
    
    return trainloader,vloader,testloader

def nn_setup(structure='densenet121',dropout=0.5,hidden1=120,lr-0.001,power=gpu):
    if structure=='vgg16':
        ...
        print("{} is not a valid model".format(structure))
...
def train_network(model,criterion,optimizer,epochs=3,printe=20,loader=trainloader,power='gpu'):
    steps=0
    running_loss=0
    for e in range(epochs):
        ...
        for ii,(inputs2,labels2) in enumerate(vloader):
                ...
                print("Epoch:{}/{}..".format(e+1,epochs),
                      "Loss:{:.4f}".format(running_loss/printe),
...
def save_checkpoint(path='checkpoint.pth',structure='densenet121',hidden1=120,dropout=0.5,lr=0.001,epochs=12):
